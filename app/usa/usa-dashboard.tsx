...
function adjustmentsFor(sale: Sale): InvoiceAdjustment[] {
  try {
    ...
    return Array.isArray(parsed) ? parsed.filter((item) => item.kind !== "SALE_EDIT") : [];
  } catch {
    return [];
  }
}

function saleEditsFor(sale: Sale): InvoiceAdjustment[] {
  try {
    const parsed = JSON.parse(sale.invoiceAdjustments || "[]") as InvoiceAdjustment[];
    return Array.isArray(parsed) ? parsed.filter((item) => item.kind === "SALE_EDIT") : [];
  } catch {
...
export default function UsaDashboard({ initialSales = [] }: { initialSales?: Sale[] }) {
  ...
  async function deleteCatalogRecord(endpoint: string, id: number, label: string, onDeleted: () => void) {
    if (!window.confirm(`¿Eliminar ${label}? Esta acción solo continuará si no tiene operaciones relacionadas.`)) return;
    try {
      const response = await fetch(`${endpoint}?id=${encodeURIComponent(id)}`, { method: "DELETE" });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || `No se pudo eliminar ${label}.`);
      onDeleted();
    } catch (error) {
      window.alert(error instanceof Error ? error.message : `No se pudo eliminar ${label}.`);
    }
  }
  ...
  function chooseSaleWarehouse(value: string) {
    ...
    if (value.startsWith("legacy:")) {
      ...
      setForm((current) => ({ ...current, warehouse: legacyName, shipTo: "" }));
      return;
    }
    const selected = coldStorages.find((item) => String(item.id) === value);
      ...
      {section === "catalogs" && <section className="erp-content">
        ...
        {partnerTypeFilter === "PRODUCT" ? <section className="sales-panel catalog-panel">
            ...
            <tbody>{productCatalogGroups.map((group) => {
              ...
              return <Fragment key={group.key}>
                ...
                {expanded && <tr className="product-variants-row" id={detailsId}><td colSpan={4}><div className="product-variants-list">
                  <div className="product-variant-head"><span>Presentación</span><span>Tamaño</span><span>Etiqueta</span><span>Acciones</span></div>
                  {group.variants.map((variant) => <div className="product-variant-item" key={variant.id}><span>{variant.presentation || "—"}</span><span>{variant.size || "—"}</span><span>{variant.label || "—"}</span><button type="button" className="delete-button" onClick={() => void deleteCatalogRecord("/api/usa/products", variant.id, `la variante de ${variant.name}`, () => setProducts((current) => current.filter((item) => item.id !== variant.id)))}>Eliminar</button></div>)}
                </div></td></tr>}
              </Fragment>;
            })}</tbody></table></div>
          ...
          <div className="table-wrap catalog-table"><table><thead><tr><th>Nombre</th><th>Dirección</th><th>Teléfono</th><th></th></tr></thead>
            <tbody>{coldStorages.map((coldStorage) => <tr key={coldStorage.id}><td><strong>{coldStorage.name}</strong></td><td>{coldStorage.address}</td><td>{formatPhone(coldStorage.phone)}</td><td><div className="row-actions"><button type="button" className="edit-button" onClick={() => openColdStorageForm("catalog", coldStorage)}>Editar</button><button type="button" className="delete-button" onClick={() => void deleteCatalogRecord("/api/usa/cold-storages", coldStorage.id, `la bodega ${coldStorage.name}`, () => setColdStorages((current) => current.filter((item) => item.id !== coldStorage.id)))}>Eliminar</button></div></td></tr>)}</tbody></table></div>
          ...
          <div className="table-wrap catalog-table"><table><thead><tr><th>Nombre</th><th>PACA #</th><th>TAX ID #</th><th>Blue Book #</th><th>DUNS & Bradstreet #</th><th>Dirección</th>{partnerTypeFilter === "CUSTOMER" && <th>Comprador</th>}<th>{partnerTypeFilter === "SUPPLIER" ? "Contacto para pagos" : "Contacto para cobros"}</th><th></th></tr></thead>
            <tbody>{partners.filter((partner) => partner.partnerType === partnerTypeFilter).map((partner) => <tr key={partner.id}><td><strong>{partner.name}</strong>{MEXICAN_STATES.some((s) => s.code === partner.stateCode) && <span style={{marginLeft:"0.4rem",fontSize:"0.65rem",fontWeight:700,background:"#16a34a",color:"#fff",borderRadius:"3px",padding:"1px 5px",verticalAlign:"middle",letterSpacing:"0.05em"}}>MX</span>}</td><td>{partner.pacaNumber || "Pendiente"}</td><td>{partner.taxId || "Pendiente"}</td><td>{partner.blueBookNumber || "Pendiente"}</td><td>{partner.dunsNumber || "Pendiente"}</td><td><strong>{[partner.street, partner.exteriorNumber && `#${partner.exteriorNumber}`, partner.interiorNumber && `Int. ${partner.interiorNumber}`].filter(Boolean).join(" ") || "Pendiente"}</strong><small>{partner.city}, {partner.stateCode} {partner.postalCode}</small></td>{partnerTypeFilter === "CUSTOMER" && <td><strong>{partner.buyerName || "Pendiente"}</strong><small>{partner.buyerEmail || ""}{partner.buyerOfficePhone ? ` · ${formatPhone(partner.buyerOfficePhone)}` : ""}</small></td>}<td><strong>{partner.contactName}</strong><small>{partner.contactEmail} · {formatPhone(partner.contactPhone)}</small></td><td><div className="row-actions"><button type="button" className="edit-button" onClick={() => void editPartner(partner)}>Editar</button><button type="button" className="delete-button" onClick={() => void deleteCatalogRecord("/api/usa/partners", partner.id, `${partner.partnerType === "CUSTOMER" ? "el cliente" : "el proveedor"} ${partner.name}`, () => setPartners((current) => current.filter((item) => item.id !== partner.id)))}>Eliminar</button></div></td></tr>)}</tbody></table></div>
      ...
      {partnerModal && <div className={`modal-backdrop ${partnerTarget ? "modal-backdrop-elevated" : ""}`}><form className="sale-modal partner-modal" onSubmit={savePartner}>
        ...
        <section className="form-section address-section"><div className="form-section-heading"><span>2</span><div><h3>Dirección</h3>{!isMexicanPartner && <p>Selecciona primero el estado para habilitar sus ciudades.</p>}</div></div><div className="form-grid partner-grid">
          {isMexicanPartner ? <>
            ...
            <label>Ciudad *<input required disabled={!partnerForm.stateCode || citiesLoading} list="partner-city-options" value={partnerForm.city} onChange={(e) => setPartnerForm({...partnerForm, city:e.target.value})} placeholder={citiesLoading ? "Cargando ciudades…" : "Selecciona o escribe una ciudad"} /><datalist id="partner-city-options">{cities.map((city) => <option key={city} value={city} />)}</datalist></label>
...
{form.operationType === "IMPORTED_INVENTORY" && saleLineItems.length > 0 && <div className="selected-inventory-sale selected-inventory-sale-final"><div><strong>Productos seleccionados</strong><button type="button" className="secondary-button" onClick={() => { const group = inventoryLoadGroups.find((item) => item.lots.some((lot) => lot.id === Number(form.inventoryLotId))); if (group) openInventorySaleLoad(group); }}>Editar detalle</button></div>{saleLineItems.map((item) => <article key={`${item.inventoryLotId}-${item.product}`}><span className="product-alias-chip">{productAlias(item.product)}</span><div><strong>{item.product}</strong><small>{[item.size, item.presentation, item.label].filter(Boolean).join(" - ") || "Sin detalle"}</small></div><span><small>Cajas</small><b>{number.format(Number(item.quantity || 0))}</b></span><span><small>Precio venta</small><b>{money.format(Number(item.unitPrice || 0))}</b></span><span><small>UTILIDAD/CAJA USD</small><b>{money.format(Number(item.unitPrice || 0) - Number(item.purchasePrice || 0))}</b></span><span><small>Total</small><b>{money.format(Number(item.quantity || 0) * Number(item.unitPrice || 0))}</b></span></article>)}</div>}
      ...
      {coldStorageModal && <div className="modal-backdrop modal-backdrop-elevated"><form className="sale-modal product-modal" onSubmit={saveColdStorage}>
        ...
        <section className="form-section"><div className="form-grid"><label>Nombre de la bodega *<input required value={coldStorageForm.name} onChange={(e) => setColdStorageForm({...coldStorageForm, name:e.target.value})} /></label><label>Estado *<select required value={coldStorageForm.stateCode} onChange={(e) => void changeColdStorageState(e.target.value)}><option value="">Selecciona un estado</option>{states.map((state) => <option key={state.code} value={state.code}>{state.name}</option>)}</select></label><label>Ciudad *<input required disabled={!coldStorageForm.stateCode || citiesLoading} list="warehouse-city-options" value={coldStorageForm.city} onChange={(e) => setColdStorageForm({...coldStorageForm, city:e.target.value})} placeholder={citiesLoading ? "Cargando ciudades..." : "Selecciona o escribe una ciudad"} /><datalist id="warehouse-city-options">{cities.map((city) => <option key={city} value={city} />)}</datalist></label><label>Calle *<input required value={coldStorageForm.street} onChange={(e) => setColdStorageForm({...coldStorageForm, street:e.target.value})} /></label><label>Número exterior *<input required value={coldStorageForm.exteriorNumber} onChange={(e) => setColdStorageForm({...coldStorageForm, exteriorNumber:e.target.value})} /></label><label>Número interior<input value={coldStorageForm.interiorNumber} onChange={(e) => setColdStorageForm({...coldStorageForm, interiorNumber:e.target.value})} /></label><label>P.O. / ZIP Code *<input required value={coldStorageForm.postalCode} onChange={(e) => setColdStorageForm({...coldStorageForm, postalCode:e.target.value})} /></label><label>Teléfono *<input required type="tel" inputMode="numeric" maxLength={14} placeholder="(XXX)-XXX-XXXX" value={formatPhone(coldStorageForm.phone)} onChange={(e) => setColdStorageForm({...coldStorageForm, phone:digitsOnly(e.target.value)})} /></label></div></section>
      ...
      {productDetailSale && <div className="modal-backdrop modal-backdrop-elevated"><section className="sale-modal product-detail-modal">
        ...
        {saleEditsFor(productDetailSale).length > 0 && <div className="invoice-adjustment-history"><strong>Historial de modificaciones</strong>{saleEditsFor(productDetailSale).map((edit) => <article key={edit.number}><div><b>{edit.number}</b><span>{new Date(edit.createdAt).toLocaleString("es-MX")}</span></div><p>{edit.notes}</p><ul>{(edit.fieldChanges || []).map((change) => <li key={`${edit.number}-${change.field}`}>{change.field}: {String(change.before || "Vacío")} → {String(change.after || "Vacío")}</li>)}</ul></article>)}</div>}
      ...
      {socPreview && <div className="modal-backdrop invoice-preview-backdrop"><section className="sale-modal invoice-preview-modal">
        ...
        {socPreview.canceledAt && <div className="canceled-record-notice"><strong>Venta cancelada</strong><span>{socPreview.canceledBy} · {socPreview.cancellationReason}</span></div>}<article className="print-document commercial-document soc-document">
          ...
          <div className="document-parties three-columns"><section><h4>CUSTOMER:</h4><strong>{socPreview.customer}</strong>{partnerAddress(documentCustomer).map((line) => <span key={line}>{line}</span>)}</section><section><h4>SHIP TO:</h4><strong>{warehouseShipTo(documentWarehouse) || "Destination pending"}</strong></section><section><h4>WAREHOUSE:</h4><strong>{documentWarehouse?.name || socPreview.warehouse}</strong>{documentWarehouse && <><span>{documentWarehouse.address}</span><span>PH: {formatPhone(documentWarehouse.phone)}</span></>}</section></div>
          <dl className="soc-order-meta"><div><dt>P.O. Date</dt><dd>{formatDocumentDate(socPreview.saleDate)}</dd></div><div><dt>P.O. #</dt><dd>{socPreview.purchaseOrder || "N/A"}</dd></div><div><dt>P.U. #</dt><dd>{socPreview.pickupNumber}</dd></div><div><dt>Buyer</dt><dd>{documentCustomer?.contactName || "Pending"}</dd></div><div><dt>Ship Terms</dt><dd>{warehouseShipTo(documentWarehouse) || "Destination pending"}</dd></div><div><dt>Payment Terms</dt><dd>21 Days</dd></div></dl>
          <table className="document-items"><thead><tr><th>Description</th><th>Size</th><th>Label</th><th>Unit</th><th className="numeric">Qty</th><th className="numeric">Unit Price</th><th className="numeric">Total</th></tr></thead><tbody>{documentItems.map((item, index) => <tr key={index}><td>{item.product}</td><td>{item.size || "—"}</td><td>{item.label || "—"}</td><td>{item.presentation || "—"}</td><td className="numeric">{number.format(item.quantity)}</td><td className="numeric">{money.format(item.unitPrice)}</td><td className="numeric">{money.format(item.quantity * item.unitPrice)}</td></tr>)}{Array.from({ length: Math.max(0, 5 - documentItems.length) }).map((_, index) => <tr className="empty-item-row" key={`empty-${index}`}><td /><td /><td /><td /><td /><td /><td /></tr>)}</tbody><tfoot><tr><td colSpan={6}>TOTAL:</td><td className="numeric">{money.format(documentTotal)}</td></tr></tfoot></table>
          <p className="amount-words">{amountInWords(documentTotal)}</p>
